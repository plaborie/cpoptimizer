...
dvar sequence shiftSeq[w in Workers] 
  in all(a in Alternatives: a.worker==w) alt[a];
...
subject to {
  ...
  // Worker unary capacity
  forall (w in Workers) {
    noOverlap(shiftSeq[w]);
  }    
}
...
execute {
  ...
  for (var w in Workers) {
    ...
    sLoad += wLoad;
    nShifts += wShifts;
    var avgWorkedLoad = wLoad / wShifts;
  	writeln("Average Load=", Opl.round(avgWorkedLoad),"%");
  }
  var avgLoad = sLoad / nShifts;
  writeln("Overall Average Load=", Opl.round(avgLoad),"%");
}