...
dvar int violated[k in 0..pmax] in 0..1;
...
minimize sum(p in 1..pmax) violated[p];

subject to {
  x == 7; // Target position is 7: for S=100 this is not possible without violating some constraints
  ...
  forall(p in 1..pmax) {
    ...
    (violated[p]==0) => (v[p] == v[k[<p,0>]] + v[k[<p,1>]]);
    
    // Values can be strictly ordered
    v[p-1] < v[p];         
  }    
}  
