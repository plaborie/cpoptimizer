...
namespace SchedPFlowShop
{
    ...
    public static void Main(String[] args)
    {
        ...
        for (int j = 0; j < nbMachines; j++)
        {
            seqs[j] = cp.IntervalSequenceVar(machines[j].ToArray(), jobIds);
            cp.Add(cp.NoOverlap(seqs[j]));
            if (0 < j)
            {
                for (int i = 0; i < nbJobs; i++)
                {
                    cp.Add(cp.Eq(cp.TypeOfPrevious(seqs[j], machines[j][i], nbJobs),
                                 cp.TypeOfPrevious(seqs[0], machines[0][i], nbJobs)));
                }
            }
        }
