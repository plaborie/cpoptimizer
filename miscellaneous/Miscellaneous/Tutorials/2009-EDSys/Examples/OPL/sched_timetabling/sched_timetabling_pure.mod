...
{int} Duration = { u | <c,d,u,n> in RequirementSet };
range DisciplineId = 0..card(Discipline)-1;
...
int MaxTime = (((sum(i in InstanceSet) i.Duration) + DayDuration) div DayDuration)*2*DayDuration;
...
// zero at night, one during day
stepFunction nightAndDay = stepwise(t in 0..MaxTime div DayDuration) 
 { (t mod 2) == 1 -> t*DayDuration ; 0 };
...
execute {
   ...
   writeln("nightAndDay = ", nightAndDay);
   writeln("DisciplineId = ", DisciplineId);
...
int showBugExtractionForbidExtent = false;
...
subject to { 
  forall(i in InstanceSet) {
    ...
    if(showBugExtractionForbidExtent != 0)
     forbidExtent(courses[i], nightAndDay);
  }
  if(showBugExtractionForbidExtent == 0)
    forall(i in InstanceSet) 
      forbidExtent(courses[i], nightAndDay);
  ...
  makespan >= ((max(c in Class) sum(r in InstanceSet : r.Class == c) r.Duration) div DayDuration)*2*DayDuration;
  
  forall(i in  InstanceSet)
     classTeacher[i.Class,i.discipline] == teacher[i];

  // ensure a course starts and ends the same halfday
  //$doc:_halfday_ct
  forall(i in InstanceSet : i.Duration > 1)
    (startOf(courses[i]) div HalfDayDuration) == ((endOf(courses[i])-1) div HalfDayDuration);
    
  // ensure that the morning disciplines ends in the morning
  //$doc:_morningonly_ct
...
execute GANTT {
  ...
  var ganttCPO = new GanttCPO(thisOplModel,
                              "FlowShop",
  						      thisOplModel.courses.start, 
  						      thisOplModel.courses.end);


  ganttCPO.addIntervalVarMap("byComputerType");
  ganttCPO.addIntervalVarMap("activity");
  ganttCPO.addIntervalSequenceVarMap("resource");
  ganttCPO.addCumulFunctionExpr("cumul", 1);
  
  ganttCPO.display(false);
}
*/
