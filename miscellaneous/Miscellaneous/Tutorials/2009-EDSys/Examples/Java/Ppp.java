...
public class Ppp {
    ...
    public static void main(String[] args) {
	    ...
	    IloIntVar[] dvars = Flatten(timePeriod);
		...
		for (int j = i + 1; j < numBoats; j++) {
		    IloIntVar meet = cp.intVar(0, numPeriods - 1);
		    for (int p = 0; p < numPeriods; p++)
			cp.add(cp.imply(cp.eq(visits[i][p], visits[j][p]),
					cp.eq(meet,p)));
		}
	    } 
	    ...
	    //
	    boolean ok = false;
	    cp.startNewSearch(cp.searchPhase(dvars));
	    while (cp.next()) {
		ok = true;
		System.out.print("[");
		for (int i=0; i< numPeriods*numBoats; i++)
		    System.out.print("["+(int)cp.getValue(dvars[i])+"]");
		System.out.println("]");
