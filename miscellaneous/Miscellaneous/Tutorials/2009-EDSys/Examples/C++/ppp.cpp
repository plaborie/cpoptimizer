...
int main(int argc, const char * argv[]) {
  ...
  try {
    ...
    IloIntVarArray dvars = Flatten(timePeriod);
    ...
    for (IloInt i = 0; i < numBoats; i++) {
      for (IloInt j = i + 1; j < numBoats; j++) {
        char name[20];
        sprintf(name, "M%ld,%ld", i, j);
        IloIntVar meet(env, 0, numPeriods - 1, name);
        for (IloInt p = 0; p < numPeriods; p++)
          mdl.add(IloIfThen(env, visits[i][p] == visits[j][p], meet == p));
      }
    } 
    ...
    //
    IloCP cp(mdl);
    if (cp.solve(IloSearchPhase(env, dvars))) {
