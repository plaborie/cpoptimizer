...
#ifndef _DTC_ITALIANO_GEN_H
#define _DTC_ITALIANO_GEN_H

#include <LEP/dynamic_graphs/dga_base.h>
#include <LEDA/node_matrix.h>

const string ITALIANO_GEN_NAME		= "Italiano Gen";
const string ITALIANO_GEN_DESCRIPTION	= string("A generalized version of a dynamic ")+
    "transitive closure algorithm developed by Italiano. The data structure suports "+
    "edge insertions and edge deletions in directed graphs.";
const string ITALIANO_GEN_VERSION	= "2.0";
const string ITALIANO_GEN_TIMESTAMP	= "October 1997";
const string ITALIANO_GEN_AUTHORS	= "Tobias Miller";
const string ITALIANO_GEN_NODES_YES	= "There is a path between these two nodes.";
const string ITALIANO_GEN_NODES_NO	= "There is no path between these two nodes.";

/*{\Manpage {dtc_italiano_gen} {} {Generalized version of Italiano's dynamic transitive closure algorithm} {DTC}}*/
class dtc_italiano_gen : public dga_base
  {
/*{\Mdefinition
An instance |\Mvar| of type |\Mname| dynamically maintains the transitive closure of a directed graph $G$.
The data type is the generalization of a partially dynamic transitive closure algorithm developed by Italiano. It efficiently supports
sequences of edge insertions or edge deletions.

The data structure supports [[insert_edge]], [[remove_edge]], [[path]] and [[desc]] operations. 
It handles mixed sequences of [[insert_edge]] and [[remove_edge]] operations. But it is not fully dynamic. It
only efficiently supports sequences of either [[insert_edge]] or [[remove_edge]] operations. }*/
...
Note that several dynamic graph algorithms can be maintained with the same message graph.}*/
    public:
/*{\Mcreation}*/     
      dtc_italiano_gen(msg_graph &G) : dga_base(&G) { update(&G); }
...
/*{\Mtext \medskip \headerline{Redefined public queries}}*/
      bool query(node v, node w) { return Desc(v,w); }
...
/*{\Mop |Stand-alone|: Like |\Mvar|.[[path(v, w, Path, Cut)]].}*/
      bool query(node v, node w, list<edge> &Path, list<edge> &Cut)
        { return path(v, w, Path, Cut); }
/*{\Mop |Stand-alone|: Like |\Mvar|.[[path(v, w, Path, Cut)]].}*/
      bool path(node v, node w) { return Desc(v,w); }
...
/*{\Mop |Stand-alone|: Assigns to |V| the descendant vertices of vertex |v| in |G|.}*/
      void scc_graph(node v, list<node> &V, list<edge> &E);
/*{\Mop |Stand-alone|: Assigns all vertices and edges in sparse graph stored with the strongly connected component that 
contains |v| to |V| and |E|.}*/
      string name() const { return ITALIANO_GEN_NAME; }
      string description() const { return ITALIANO_GEN_DESCRIPTION; }
      string version() const { return ITALIANO_GEN_VERSION; }
      string timestamp() const { return ITALIANO_GEN_TIMESTAMP; }
      string authors() const { return ITALIANO_GEN_AUTHORS; }
      string nodes_yes() const { return ITALIANO_GEN_NODES_YES; }
      string nodes_no()  const { return ITALIANO_GEN_NODES_NO; }  

    private:
      struct scc
        {
          dtc_italiano_gen* This;
          GRAPH<node, edge> G;      /* subgraph and sparse certificate */
          list<edge>        In;     /* list of in-coming edges */
          list<edge>        Out;    /* list of out-going edges */
          node_array<edge>  Parent; /* parent references */
          node_array<int>   Time;
          node_array<int>   Visit;
          list_item         Item, RemItem;
          int               Flag;
          
          scc(dtc_italiano_gen* This, node n);
          scc(dtc_italiano_gen* This, list<node> &L);
          
          bool path(node, node, list<node>&);
          bool path(node, node, list<edge>&);
          void hook(node, edge);
          bool search_hook(node, list_item);
          bool compute_sparse();
          void compute_break(list<scc*>&);
          void inline set_visit(int i) { node v; forall_nodes(v,G) Visit[v]=i; }
        };

      int                 Time, Reset; /* 1 after insert, 0 otherwise */
      node_array<scc*>    SCC;         /* maps vertex to scc */ 
      node_matrix<bool>   Desc;        /* descendant matrix. */
      node_array<node>    GVIndex;     /* index into vertex set of G */
      edge_map<edge>      GEIndex;     /* index into edge set of G */
      edge_map<list_item> InIndex;     /* index into In lists */
      edge_map<list_item> OutIndex;    /* index into Out lists */
      edge_map< list<node> > EIndex;
      
      void reset();
      void meld(node, edge);
      void merge(node);
      void split(scc*, list<scc*>&, edge);
      
      friend struct scc;
  };

/*{\Mimplementation
The data structure generalizes two partially dynamic transitive closure algorithms developed by Italiano. Italiano's data structure handles 
edge deletions in directed acyclic graphs only. We have amended Italiano's work to handle edge deletions in any directed graph. For further 
details see G.F.~Italiano, ``Amortized efficiency of a path retrieval data structure'', {\em Theoretical Computer Science}, 48 (1986), 273-281 
and G.F.~Italiano, ``Finding paths and deleting edges in directed acyclic graphs'', {\em Information Processing Letters} 28:1 (1988), 5-11. 

The data structure can be initialized in $O(n*(n+m))$ time and requires $O(n^2+m)$ space, 
where |n| is the number of vertices and |m| the number of edges in the graph. Any sequence of [[insert_edge]] operations requires
$O(n*(n+m))$ time, where |m| is the final number of edges in the graph. A sequence of [[remove_edge]] operations requires $O((n+m)^2)$ worst case and
$O(n*(n+m))$ expected time, where |m| is the initial number of edges in the graph. A [[path(node, node)]] query requires $O(1)$, 
a [[desc]] or a [[path(node, node, list)]] query $O(n)$ and a [[path(node, node, list, list)]] query $O(m+n)$ time.}*/
#endif _DTC_ITALIANO_GEN_H
