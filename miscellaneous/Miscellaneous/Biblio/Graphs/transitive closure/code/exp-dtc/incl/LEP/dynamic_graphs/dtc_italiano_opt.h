...
#ifndef _DTC_ITALIANO_OPT_H
#define _DTC_ITALIANO_OPT_H

#include <LEP/dynamic_graphs/dga_base.h>
#include <LEDA/node_matrix.h>

const string ITALIANO_OPT_NAME        = "Italiano Opt";
const string ITALIANO_OPT_DESCRIPTION = string("An optimized version of a dynamic ")+
    "transitive closure algorithm developed by Italiano. The data structure supports "+
    "edge insertions in directed graphs and edge deletions in directed acyclic graphs.";
const string ITALIANO_OPT_VERSION     = "1.0";
const string ITALIANO_OPT_TIMESTAMP   = "April 1998";
const string ITALIANO_OPT_AUTHORS     = "Tobias Miller";
const string ITALIANO_OPT_NODES_YES   = "There is a path between these two nodes.";
const string ITALIANO_OPT_NODES_NO    = "There is no path between these two nodes.";

/*{\Manpage {dtc_italiano_opt} {} {Optimized version of Italiano's dynamic transitive closure algorithm} {DTC}}*/
class dtc_italiano_opt : public dga_base
  {
/*{\Mdefinition
An instance |\Mvar| of type |\Mname| dynamically maintains the transitive closure of a directed graph $G$.
The data type is an optimized version of a partially dynamic transitive closure algorithm developed by Italiano. It efficiently supports
...
Note that several dynamic graph algorithms can be maintained with the same message graph.}*/
    public:
/*{\Mcreation}*/     
      dtc_italiano_opt(msg_graph* G) : dga_base(G) { update(G); }
...
each [[remove_edge]] operation that is preceded by an [[insert_edge]] operations.}*/
      string nodes_yes() const { return ITALIANO_OPT_NODES_YES; }
      string nodes_no()  const { return ITALIANO_OPT_NODES_NO; }  
      string name() const { return ITALIANO_OPT_NAME; }
      string description() const { return ITALIANO_OPT_DESCRIPTION; }
      string version() const { return ITALIANO_OPT_VERSION; }
      string authors() const { return ITALIANO_OPT_AUTHORS; }
      string timestamp() const { return ITALIANO_OPT_TIMESTAMP; }

    protected:
      int               Time, Reset; /* 1 after insert, 0 otherwise */
      node_matrix<edge> Parent;      /* parent references */
      node_matrix<bool> Desc;        /* descendant matrix */
      node_matrix<int>  ResetTime;
...
/*{\Mimplementation
The data structure realizes an optimized version of two partially dynamic transitive closure algorithms developed by Italiano. For further 
details see G.F.~Italiano, ``Amortized efficiency of a path retrieval data structure'', {\em Theoretical Computer Science}, 48 (1986), 
273-281 and G.F.~Italiano, ``Finding paths and deleting edges in directed acyclic graphs'', {\em Information Processing Letters} 28:1 (1988), 5-11. 
