...
bool check(ordered_slist2<void> &l) {
  ordered_slist2<void>::iterator i(l.begin());
  ordered_slist2<void>::iterator j(i);
  ++i;
...
int main(int argc, char *argv[]) {
  ...
  char *descriptions[]={
    ...
    " -n     --no-checking             do not perform validation checking",
    NULL
  };
  ...
  // construct the list
  ordered_slist2<void> l(size);
  // perform the operations
  if(checking && !check(l)) {
    cout << "failure during construction : ";
    for(ordered_slist2<void>::iterator i(l.begin());
	i!=l.end();++i) {
  ...
  while(over-- > 0) {
    ...
    if(v >= 500 && !l.empty()) {
      // perform insert
      ordered_slist2<void>::iterator x(l.begin()); 
