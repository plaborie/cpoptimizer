...
/* generator of acyclic + negative cycles networks for the shortest paths
   problem */
...
long   ll = 10000,    /* upper bound of the interval */
       lm = 1;        /* lower bound of the interval */
...
double pn = 0,        /* l += ln * |i-j| */ 
 ...
 /* variables for negative cycles */
 int    n_f = 0, nn_f = 0, nl_f = 0, na_f = 0;
 
 long   nn = 1,        /* number of negative cycles */
        nl = 3,        /* number of arcs in negative cycle */
        na = -1;       /* length of negative cycle */            
 long   lc,            /* current cycle length */
        lv,            /* length of cycle arc */
        *nd,           /* array for nodes */
        nk;            /* number of available nodes */
...
for ( np = 4; np < argc; np ++ )
  {
	...
	break;
       ...
       case 'n' : /* negative cycles */
 	n_f = 1;
 	if ( strlen ( args ) > 2 )
 	{  
 	switch ( args[2] )
 	  { 
 	  case 'a': /* negative cycle length */
 	    na_f = 1;
 	    na  =  (long) atof ( &args[3] );
 	    break;
 	  case 'l': /* arcs in negative cycle */
 	    nl_f = 1;
 	    nl  = (long ) atof ( &args[3] );
 	    break;
 	  case 'n': /* number of negative cycles */
 	    nn_f = 1;
 	    nn  = (long) atof ( &args[3] );
 	    break;
 	  default:  /* unknown switch  value */
 ...
 if ( n_f )
   {
     ...
     if ( nn < 1 ) nn = 1;
     if ( nl > nk ) nl = nk;
     if ( nl*nn > nk ) nn = nk / nl;
     m0 += nn * nl;
   }

/*----- printing title -----*/

printf ("c acyclic network with negative cycles for shortest paths problem\n");
...
if ( s_f ) /* additional arcs from artificial source */
  {
    seed2 = 3*seed + 1;
    init_rand ( seed2 );
    sl = sl - sm + 1;
...
for ( k = 1; k <= m - mc; k ++ )
 ...
 /* generating negative cycles */
 if ( n_f )
 {
 lv = na / nl;
 nk = n0;
 
 nd = (long*) calloc ( n0, sizeof ( long ) );
 ...
 for ( c = 0; c < nn; c++ )
 {
   ...
   /* last arc of the cycle */
   PRINT_ARC ( j, i0, na - (nl-1)*lv )
 }
 }
...
fprintf ( stderr, 
"\n'%s' - acyclic + negative cycles network generator for shortest paths problem.\n\
\n\
...
-lm#i  - #i is the lower bound on arc lengths          (default 1)\n\
...
fprintf (stderr, "No available help", argv[0], argv[0], argv[0] );
exit (0);
}
...
-sm#i  - #i is the lower bound on art. arc lengths    (default sl)\n\
      ...
      Negative cycles:\n\
 -n     - generate default negative cycles\n\
 -nl#i  - #i is the number of arcs in negative cycle   (default 3)\n\
 -nn#i  - #i is the number of negative cycles          (default 1)\n\
 -na#i  - #i is the length of negative cycle           (default -1)\n\
 \n"
*/
