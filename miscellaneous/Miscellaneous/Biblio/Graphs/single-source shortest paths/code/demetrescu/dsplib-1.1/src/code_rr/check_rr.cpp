...
// Change log:      CD000422
// Description:     RR Algorithm Checker using Bellman-Ford-Moore algorithm
...
int main(int argc, char *argv[]){
    ...
    // Create instance of RR class
    bool theNegCycleFound;
    class_rr SP(GP,theNegCycleFound,theSource);
    ...
    if ( (theResult = Compare(SP, GP, theDist, theNodeLabel)) != 0) {
        cout << "Initialization error " << theResult << " in rr\n";
        exit(1);
    }
    else cout << "\nInitialization OK.\n";
    ...
    for (int i=0; i<numOp; i++) {
        ...
        if (SP.ChangeWeight(nodo1, nodo2, theWeight) != class_rr::_OK) {
        ...
        if ( ( theResult = Compare(SP, GP, theDist, theNodeLabel) ) != 0 ) {
            cout << "Op. # " << i+1 <<  ( (theWeight > theOldWeight) ? " Increase " : " Decrease " ) << "from "
                 ...
                 << " - different " << ( theResult==1 ? "distance" : "parent" ) << " in rr\n";
            if ( theResult == 1 ) exit(1);
        }
...
// Compare results of Bellman-Ford-Moore and Ramalingam-Reps
