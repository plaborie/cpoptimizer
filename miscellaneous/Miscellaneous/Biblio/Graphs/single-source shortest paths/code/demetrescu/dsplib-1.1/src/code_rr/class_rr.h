...
// Change log:     CD000430
// Description:    class header for class_rr
...
public:
    ...
    struct EdgeInfo {
        ...
        list_item               SourceInItem;       // edge list item in target's list of in-edges in the DAG
        list_item               SourceOutItem;      // edge list item in source's list of out-edges in the DAG
    };
    ...
    struct NodeInfo {
        ...
        list<struct EdgeInfo*>  DAGInList;          // list of in-edges in the shortest paths dag
        list<struct EdgeInfo*>  DAGOutList;         // list of out-edges in the shortest paths dag
        ...
        int                     Color;              // node marking (_Increase only)
                                ...
                                class_rr(GRAPH<int,int> &inGraph, bool &outIsNegCycle, int inSource=0);
                               ~class_rr();
...
private:
    ...
    enum NodeColor {
        ...
        _Red                    // the node will change its distance from the source
                                ...
                                // Private methods
    inline bool                 _SearchEquivalentPath( NodeInfo* inNodeInf );
