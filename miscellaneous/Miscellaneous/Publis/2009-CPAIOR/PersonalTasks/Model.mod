 using CP;
 tuple Task { key int id; int loc; int dur; int smin; int smax; int dmin; 
              int f; int date; {int} ds; {int} de; } 
 {Task} Tasks = ...;
 ...
 sorted {Step} Steps[t in Tasks] = {<x,0> | x in t.ds} union {<x,1> | x in t.de};
