...
\hypersetup{%
pdftitle={Reasoning with Conditional Time-intervals},
pdfauthor={Philippe Laborie; J\'er\^ome Rogerie},
pdfkeywords={},
%bookmarksnumbered,
pdfstartview={FitH},
urlcolor=cyan,
...
%% CONSTRAINTS
\DeclareMathOperator{\ctexec}{exec}
\DeclareMathOperator{\ctspan}{span}
\DeclareMathOperator{\ctalternative}{alternative}
\DeclareMathOperator{\ctstartbeforestart}{startBeforeStart}
\DeclareMathOperator{\ctstartbeforeend}{startBeforeEnd}
\DeclareMathOperator{\ctendbeforestart}{endBeforeStart}
\DeclareMathOperator{\ctendbeforeend}{endBeforeEnd}
\DeclareMathOperator{\ctstartatstart}{startAtStart}
\DeclareMathOperator{\ctstartatend}{startAtEnd}
\DeclareMathOperator{\ctendatstart}{endAtStart}
\DeclareMathOperator{\ctendatend}{endAtEnd}
...
\begin{center} 
{\sc ILOG\ Technical report number 08-001}
...
\begin{center}
{\Large {\bf Reasoning with Conditional Time-intervals}} \\
\vspace{1.5cm}
{\large Philippe Laborie and J\'er\^ome Rogerie} \\
\vspace{0.7cm}
...
\begin{itemize}
  ...
  \item set of operations that can be processed in different temporal modes (for instance in series or in parallel), 
...
\item constraints in Constraint-Based Scheduling can be extended to efficiently propagate on {\em time-interval variables} without impacting their algorithmic complexity.
\end{itemize}
...
Execution status of time-interval variables can be further restricted by logical constraints. The {\bf execution constraint} $\ctexec(a)$ states that a given time-interval variable must be executed. The semantics of the execution constraint on a fixed time-interval $\underline{a}$ is:
\[\ctexec(\underline{a}) \overset{d}{\leftrightarrow} \big(x(\underline{a})=1 \big)\]

In the basic model described in this paper, we only consider unary and binary logical constraints between execution statuses, that is, constraints of the form of 2-SAT clauses over execution statuses: $[\neg]\ctexec(a)$ or $[\neg]\ctexec(a) \lor [\neg]\ctexec(b)$. For example if $a$ and $b$ are two conditional time-intervals such that when time-interval $a$ is executed then $b$ must be executed too, it can be modelled by the constraint $\neg \ctexec(a) \lor \ctexec(b)$.

\subsection{Temporal Constraints}

The temporal constraint network consists of a Simple Temporal Network (STN) extended to the execution statuses. For instance, a temporal relation $\ctendbeforestart(a,b)$ states that {\em if both time-intervals $a$ and $b$ are executed} then the end of $a$ must occur before the start of $b$.

The semantics of the relation $\text{TC}(\underline{a},\underline{b},z)$ on a pair of fixed time-intervals $\underline{a}$, $\underline{b}$ and for a delay value $z$ depending on the temporal relation type $\text{TC}$ is given on Table \ref{tab:temporal-constraint-semantics}. 
\begin{table}[ht]
	\centering
		\begin{tabular}{|l|l|} \hline
		{\bf Relation} & {\bf Semantics} \\ \hline
		$\ctstartbeforestart$ & $x(\underline{a}) \land x(\underline{b}) \Rightarrow s(\underline{a})+z \leq s(\underline{b})$ \\ \hline
		$\ctstartbeforeend$   & $x(\underline{a}) \land x(\underline{b}) \Rightarrow s(\underline{a})+z \leq e(\underline{b})$ \\ \hline
		$\ctendbeforestart$   & $x(\underline{a}) \land x(\underline{b}) \Rightarrow e(\underline{a})+z \leq s(\underline{b})$ \\ \hline
		$\ctendbeforeend$     & $x(\underline{a}) \land x(\underline{b}) \Rightarrow e(\underline{a})+z \leq e(\underline{b})$ \\ \hline
		$\ctstartatstart$     & $x(\underline{a}) \land x(\underline{b}) \Rightarrow s(\underline{a})+z = s(\underline{b})$    \\ \hline
		$\ctstartatend$       & $x(\underline{a}) \land x(\underline{b}) \Rightarrow s(\underline{a})+z = e(\underline{b})$    \\ \hline
		$\ctendatstart$       & $x(\underline{a}) \land x(\underline{b}) \Rightarrow e(\underline{a})+z = s(\underline{b})$    \\ \hline
		$\ctendatend$         & $x(\underline{a}) \land x(\underline{b}) \Rightarrow e(\underline{a})+z = e(\underline{b})$    \\ \hline
		\end{tabular}
...
The {\bf alternative time-intervals constraint} $\ctalternative(\underline{a_0}, \{\underline{a_1},...,\underline{a_n}\})$ holds if and only if: 
\begin{eqnarray*}
\neg x(\underline{a_0}) & \Leftrightarrow & \forall i \in [1,n],  \neg x(\underline{a_i}) \nonumber \\
...
\begin{eqnarray*}
\ctexec(a); && \ctexec(b); \\
\ctstartatstart(a, a_1); && \ctstartatstart(a, a_2); \\
\ctstartatstart(b, b_1); && \ctstartatstart(b, b_2); \\
\ctendatend(a, a_1); && \ctendatend(a, a_2); \\
\ctendatend(b, b_1); && \ctendatend(b, b_2); \\
\ctendbeforestart(a_1, b_1); && \ctendbeforestart(b_2, a_2); \\
\neg \ctexec(a_1) \lor \neg \ctexec(a_2); && \ctexec(a_1) \lor \ctexec(a_2);\\
\neg \ctexec(b_1) \lor \neg \ctexec(b_2); && \ctexec(b_1) \lor \ctexec(b_2);\\
\neg \ctexec(a_1) \lor \ctexec(b_1); && \ctexec(a_1) \lor \neg \ctexec(b_1);
\end{eqnarray*}
\normalsize
...
\item Logical constraint are represented by dotted arrows and denote an implication relation (for instance $\ctexec(a) \Rightarrow \ctexec(b)$). In case the arrow starts or ends at a cross, the corresponding end-point of the implication is the negation of the execution status.
...
Suppose an activity $a$ can be executed in $n$ possible modes $\{a_i\}_{i \in [1,n]}$ with duration $da_i$ for mode $a_i$ and an activity $b$ can be executed in $m$ possible modes $\{b_j\}_{j \in [1,m]}$ with duration $db_j$ for mode $b_j$. Furthermore, there are some mode incompatibility constraints $(i,j)$ specifying that mode $a_i$ for $a$ is incompatible with mode $b_j$ for $b$. This model is represented on Figure \ref{fig:model:altmodes}, incompatibilities $(i,j)$ are modelled by implications $\ctexec(a_i) \Rightarrow \neg \ctexec(b_j)$ (that is: $\neg \ctexec(a_i) \lor \neg \ctexec(b_j)$). Of course, in practical applications, time-interval variables $a_i$ and $b_j$ will require some conjunction of resources but, as mentioned in the introduction, this is out of the scope of the present paper. If binary logical constraints are insufficient to model the compatibility rules, the execution status of time-intervals $\ctexec(a_i)$ can also be used in standard constraint programming constructs such as n-ary logical or arithmetic expressions or table constraints \cite{Bessiere1997}.

\begin{figure}[ht]
\centering
\includegraphics[width=8cm,keepaspectratio,clip]{figures/model_altmodes.pdf}
...
Figure \ref{fig:model:seriesparallel} depicts a model where a job is composed of two operations $a$ and $b$ that can be executed either in series or in parallel (in this case, both operations are constrained to start at the same date). This is modelled by two alternatives $\ctalternative(a, \{a_1,a_2\})$ and $\ctalternative(b, \{b_1,b_2\})$ with $(a_1,b_1)$ describing the serial and $(a_2,b_2)$ describing the parallel execution. Logical constraints $\ctexec(a_i) \Leftrightarrow \ctexec(b_i)$ are added to ensure a consistent selection of the alternative.

\begin{figure}[ht]
\centering
\includegraphics[width=8cm,keepaspectratio,clip]{figures/model_seriesparallel.pdf}
...
Figure \ref{fig:model:altrecipes} describes a set of 3 alternative recipes. The global process $a$ is modelled as an alternative of the 3 recipes $r_1$, $r_2$ and $r_3$. Each recipe is a time-interval variable that spans the internal operations of the recipe. Implication constraints between a recipe and some of its internal operations (for instance $\ctexec(r_3) \Rightarrow \ctexec(o_{31})$) mean that operation is not optional in the recipe. Note that the opposite implications (for instance $\ctexec(o_{31}) \Rightarrow \ctexec(r_{3})$) are part of the span constraint. This pattern can be extended to a hierarchy of spanning tasks for modelling complex work-breakdown structures in project scheduling.

\begin{figure}[ht]
\centering
\includegraphics[width=8cm,keepaspectratio,clip]{figures/model_altrecipes.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=8cm,keepaspectratio,clip]{figures/model_nooverlap.pdf}
...
All 2-SAT logical constraints between time-interval execution statuses of the form $[\neg]\ctexec(a) \lor [\neg]\ctexec(b)$ are aggregated in a {\em logical network} similar to the implication graph described in \cite{Brafman2001}. The objectives of the logical network are:
\begin{itemize}
...
Nodes $\{l_i\}_{i \in [1,n]}$ in the graph correspond to execution statuses $\ctexec(a)$ or their negation $\neg \ctexec(a)$ and an arc $l_i \rightarrow l_j$ corresponds to an implication relation between the corresponding boolean statuses. For instance a constraint $\ctexec(a) \lor \ctexec(b)$ would be associated with an arc $\neg \ctexec(a) \rightarrow \ctexec(b)$. As links $l_i \rightarrow l_j$ and $\neg l_j \rightarrow \neg l_i$ are equivalent, for each time-interval variable $a$, only one node has to be considered in the network, either the one corresponding to $\ctexec(a)$ or the one corresponding to $\neg \ctexec(a)$. Fixed execution statuses are skipped from the network as in this case binary constraints are reduced to unary constraints. Strongly connected components of the implication graph are collapsed into a single node representing the logical equivalence class and the transitive closure of the resulting directed acyclic graph is maintained as new arcs are added. The logical network becomes inconsistent when it allows to infer both relations $l_i \rightarrow \neg l_i$ and $\neg l_i \rightarrow l_i$.
...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm,keepaspectratio,clip]{figures/inference_pistons.pdf}
\caption{Example of propagation}
\label{fig:inference:pistons}
\end{figure}

Figure \ref{fig:inference:chain} illustrates another example of propagation. The model consists of a chain of $n$ identical optional operations $\{o_i\}_{i \in [1,n]}$ of duration $10$ that, if executed, need to be executed before date $25$ and are such that $\ctexec(o_{i+1}) \Rightarrow \ctexec(o_i)$. Although initially, all operations are optional, the propagation will infer that only the first two operations can be executed and will compute the exact conditional minimal start and end times for the two possibly executed operations. We are not aware of any other framework that is capable of inferring, by constraint propagation only, such type of information on purely optional activities.

\begin{figure}[ht]
\centering
\includegraphics[width=10cm,keepaspectratio,clip]{figures/inference_chain.pdf}
\caption{Example of propagation}
...
\begin{itemize} 
\item In the case of a span constraint $\ctspan(a_0, \{a_1, ..., a_n\})$, the set of implications $\ctexec(a_i) \Rightarrow \ctexec(a_0)$ are treated by the logical network and the set of arcs $\ctstartbeforestart(a_0,a_i)$ and $\ctendbeforeend(a_i,a_0)$ by the temporal network.
\item In the case of an alternative constraint $\ctalternative(a_0, \{a_1, ..., a_n\})$, the set of implications $\ctexec(a_i) \Rightarrow \ctexec(a_0)$ are treated by the logical network and the set of arcs $\ctstartatstart(a_0,a_i)$ and $\ctendatend(a_i,a_0)$ by the temporal network.
\end{itemize}
...
As illustrated on Figure \ref{fig:propagation}, coupling global constraints with logical and temporal network information allows performing stronger conjunctive deductions on the bounds of time-interval variables and inferring new logical and temporal relations. 

\begin{figure}[ht]
\centering
\includegraphics[width=8cm,keepaspectratio,clip]{figures/propagation.pdf}
\caption{General approach for propagation}
\label{fig:propagation}
\end{figure}

The search method described in \cite{Laborie2007} has been extended to handle time-interval variables. It consists of a tree search for producing an initial solution followed by a self-adapting iterative improvement method. Beside constraint propagation, the logical and temporal networks described in this paper, which are linear structures that convey important information about the structure of the problem, are also used to compute relaxations to guide tree search and to define the moves of the iterative method. Future work will consist in enhancing constraint propagation and search following this general pattern.
