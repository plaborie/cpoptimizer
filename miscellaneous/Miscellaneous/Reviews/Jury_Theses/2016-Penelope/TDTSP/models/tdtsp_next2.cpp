...
void TDTSPInstance::solve() {
  ...
  try {
    ...
    IloIntVarArray positionOf(env,_nbVertices+1);
    ...
    positionOf[_nbVertices] = IloIntVar(env, _nbVertices, _nbVertices);
    ...
    for(IloInt i=0; i< _nbVertices; i++){
      ...
      for(IloInt j=0; j<_nbVertices; j++){
        for(IloInt k=0; k< _nbTimeWindows; k++){
          ...
          costMatrix2[i][j*_nbTimeWindows+k]=_cost[i][j][k];
...
//---------------------------- CONSTRAINTS
    ...
    for(IloInt i=0; i<_nbVertices-1; i++){
      ...
      model.add(positionOf[next[i]]==positionOf[i]+1); // TRYING TO ADD THIS ONE
    ...
    for(IloInt i=0; i<_nbVertices; i++){
      ...
      IloIntExpr indexInCostMatrix2 = (locationsArray[next[i]] *_nbTimeWindows) + timeWindow[i];
      model.add(travelTime[i] == costMatrix2[i][indexInCostMatrix2]);
      model.add(time[next[i]]>= time[i] + durationsArray[locationsArray[i]] + travelTime[i]);
    }
    model.add(tw[0]==0);
    for(IloInt i=1; i<=_nbVertices; i++){
      ...
      model.add(time[i] >= time[prev[i]] + durationsArray[prev[i]] + td[locationsArray[i]]);
...
//------------------------------ OBJECTIVE
     ...
     for(IloInt i=0; i<_nbVertices; i++){
       totalDuration+=durationsArray[i];
     }
    IloIntExpr totalTravelTime= IloSum(travelTime);
    model.add(totalTravelTime + totalDuration <= time[_nbVertices]);
