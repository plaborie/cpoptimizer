...
public:
  ...
  void solve(const char* nameSol); //nameSol = NULL if no starting point
  void print(const char* nameBench, const char* nameInstance);
  IloInt getNbVertices() const { return _nbVertices; }
  ...
  ,_visits           (0)
{
  ...
  if (indexMax > benchSize) {
    std::cerr << "Not enough addresses in bench file" << std::endl; 
    std::cerr << "indexMax  = " << indexMax << std::endl;
    std::cerr << "benchSize = " << benchSize << std::endl;
  }
...
void TDTSPInstance::solve(const char* nameSol) {
  IloEnv env;
  try {
    IloModel model(env);
...
//---------------------------- CONSTRAINTS
    ...
    //TW constraints
    for(IloInt i=0; i< _nbVertices; i++){
      IloNumToNumStepFunction f(env, 0, IloIntervalMax, 0);
      f.setValue(0, INSTANCE_HORIZON, 1);
      if(_nbWindows[i]!=0){
...
//---------------------------SET "STARTING POINT" 
//not a real starting point, just constraints forcing some of the 
//characteristics of the starting point
    ...
    // }
    // else {
      // std::cout << "Adding starting point constraints of: " << nameSol << endl;
    // }
    // IloInt nbV, visit, start, index, indexPrev;
    ...
    // while (input.getline(line, lineLimit)) {
      // if (Contains(line, "## Solution:")) {
        ...
        // for(IloInt i=0; i< nbV; i++){
          ...
          // if(indexPrev != -1){
            // model.add(IloEndBeforeStart(env, stops[indexPrev], stops[index]));
          // }
          // indexPrev=index;
        // }
      // }
...
//-------------------------------SOLVE-----------------------------------------  
    ...
    */
    ...
    if (cp.next()) {
      ...
      t1 = t0 - extrTime;
      cout << "# EXTRACTION TIME: " << extrTime << endl;
      cout << "* " << t1 << " " << cp.getObjValue();
      // if (cp.getObjValue() > _nbTimeWindows*_lengthTimeWindow)
      //  cout << " -1";
      cout << endl;
      cp.setParameter(IloCP::TimeLimit, TIME_LIMIT+t0-t1);
      while (cp.next()){
        cout << "* " << cp.getInfo(IloCP::SolveTime)-t0+t1 << " " << cp.getObjValue();
        // if(cp.getObjValue() > _nbTimeWindows*_lengthTimeWindow)
        //   cout << " -1";
        cout << endl;
      }
