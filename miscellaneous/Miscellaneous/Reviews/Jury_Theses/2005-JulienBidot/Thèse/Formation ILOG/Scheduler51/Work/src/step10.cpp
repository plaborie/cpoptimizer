...
///////////////////////////////////////////////////////////////////////////////
//
// Step 10: Defining new goals
//
// Built-in goals may be not efficient with some problems.
// So build your own goal, with a user-defined selector:
//   - Select the first non bound activity
//   - Try to schedule it at its earliest start time
//
///////////////////////////////////////////////////////////////////////////////
                   ...
                   IloUnaryResource& worker)
{
  ...
  IloIntervalList breakList(env,0,14);

  // add the weekend breaks
  breakList.addPeriodicInterval(5,2,7,14);
  ...
  // set other activities who can overlap one day
  ...
  // assign it to the non-breakable and long enough activities
  ...
  // create the symmetric transition time table and fill in it
  ...
  for (i = 0; i < nbActivities; i++)
  {
    ...
    // processing time :
    // in fact, the variable is instanciated
...
PrintSolutionByArray(IloSolver solver, IloArray<IloActivity> arrayActivities)
{
  ...
  for (IloInt i = 0; i < nbActivities; i++)
  {
    ...
    IlcActivity currentActivityExtensions = scheduler.getActivity(currentActivity);
...
ILCGOAL1(ScheduleActivity,
  ...
  /********* to be filled in *********/
	return IlcScheduleOrPostpone(activity);
}
...
ILCGOAL1(SolveProblem, IlcActivitySelectorObject, actSel) {
  ...
  if (anActivityIsSelected)
	  return IlcAnd(ScheduleActivity(solver,selected),this);
...
ILOCPGOALWRAPPER0(IloSolveProblem, solver) {
  ...
  IlcActivitySelectorObject actSel (!IlcActStartVarBoundPredicate(solver)&&!IlcActPostponedPredicate(solver),IlcActStartMinEvaluator(solver) /********* to be filled in *********/	);
...
int main()
{
  ...
  IloArray<IloActivity> arrayActivities;
  IloUnaryResource worker;
  DefineProblem(model, makespan, arrayActivities, worker);

  // extracting a resolution structure
  IloSolver solver(model);

  // creating and activating the goal,
  IloGoal goalSearch = IloSolveProblem(env);
  solver.solve(goalSearch);
