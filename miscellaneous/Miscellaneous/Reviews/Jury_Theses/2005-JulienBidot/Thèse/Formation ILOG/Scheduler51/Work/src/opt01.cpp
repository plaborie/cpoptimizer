...
//
//
//  Step Opt01: Printing solutions with IloSchedulerSolution
//
...
PrintSolution(IloSchedulerSolution solution)
{
  ...
  /* To be filled in */
  for(IloSchedulerSolution::ActivityIterator ite = IloSchedulerSolution::ActivityIterator(solution); ite.ok(); ++ite)
	  env.out() << *ite << endl;
}
...
int main()
{
	...
	for (IloInt i = 0 ; i < arrayActivities.getSize() ; i++)
