...
% Derniere revision 31 mars 2002 : ajout Christian
...
de t\^aches, nombre de changements de ressources, \emph{etc.}
...
dynamique.
\item[${\cal I}$] une instance r\'ealis\'ee de ${\cal P}$ (conditions
 effectives \`a
...
$$R=3D\max_{{\cal I} \in {\cal P}} d (z^*_{{\cal I}}, z_{{\cal
...
robustesse R est grande. La mesure de robustesse en moyenne
suppose connue la distribution des diff\'erentes instances
r\'ealis\'ees de ${\cal P}$, et s'exprime alors comme une
esp\'erance~:
$$E_{{\cal I} \in {\cal P}} \left( d (z^*_{{\cal I}},z_{{\cal=
 I}}(S^{\alpha}_{\delta}) ) \right)$$
...
$\alpha$. Dans ce cas on recherche une garantie
 ...
 famille raisonnable d'algorithmes dynamiques). La mesure de la
 robustesse au pire
devient alors~:
$$R=3D\max_{{\cal I} \in {\cal P}} \max_{S \in
\Sigma^{\alpha}}
 d \left(z^*_{{\cal I}}, z_{{\cal I}}(S) \right)$$
La mesure de robustesse en moyenne peut \^etre adapt\'ee de la m\^eme
 mani\`ere.
...
$$R=3D\max_{{\cal I} \in {\cal P}} d (\tilde z({\cal P}),z_{{\cal=
 I}}(S^{\alpha}_{\delta}) )$$

    \item la mesure du niveau de service. Il s'agit d'assurer que le niveau
 de la mesure de robustesse est inf\'erieur \`a un certain seuil fix\'e
 $T({\cal
    P})$. $T({\cal P})$ est une garantie de performance explicite.

$$R=3D\max_{{\cal I} \in {\cal P}} d (\tilde z({\cal P}),z_{{\cal=
 I}}(S^{\alpha}_{\delta}) ) \le T({\cal P}))$$

Le niveau de service est un taux de service lorsque les
diff\'erentes instances sont connues par leur distribution de
probabilit\'e. La robustesse du syst\`eme global est alors li\'ee
\`a la probabilit\'e que l'ordonnancement calcul\'e assure un
niveau de performance pr\'e-\'etabli.
...
$d'$ mesure l'\'ecart entre deux \ordos\ (similarit\'e), elle est donc
 utilisable pour la mesure de flexibilit\'e introduite section \ref{flex}.
...
stabilit\'e de l'algorithme ou du processus global (voir r\'ef\'erence Sotskov \cite{sotskov})~:
$$\max_{{\cal I} \in {\cal P}} d(z_{\cal I}(\tilde S),z_{\cal=
 I}(S^{\alpha}_{\delta}))$$
...
ordonnancement sont nombreuses. Cela revient dans notre cadre \`a
...
install\'es \`a proximit\'es des usines d'assemblage. Ils d\'elivrent
...
% Mireille Jacomino et Andre Rossi Grenoble
...
% Eric Sanlaville Clermont-Ferrand, Aziz Moukrim Compiegne, Frederic Guinand Le Havre
...
\subsection{Insertion d'une t\^ache impr\'evue en gestion  de projet
...
mal \`a, certains
probl\`emes int\'eressants. C'est le cas de l'\ordo\ cyclique, non trait\'e ici, ou des perturbations sur le
crit\`ere lui-m\^eme.
...
\section{Liste des membres du groupe flexibilit\'e du GOThA}
