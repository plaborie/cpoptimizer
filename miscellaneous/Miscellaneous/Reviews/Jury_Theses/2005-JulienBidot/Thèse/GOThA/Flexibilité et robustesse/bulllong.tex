% Version longue en vue de la publication dans le bulletin ROADEF printemps 2002

% Derniere revision 6 mars 2002 :
% Ne contient pas la section complete sur les metriques !!!!
...
de {\em r\'epa\-ra\-tion\/} d'un \ordo. On peut \'egalement d\'efinir la 
...
Dans l'\'etude de la robustesse a priori, $\alpha$, $\delta$ et ${\cal P}$
...
devient alors
$$\max_{{\cal I} \in {\cal P}} \max_{S \in \Sigma^{\alpha}}
...
pour la
mesure de flexibilit\'e introduite section \ref{flex}.
...
dates de disponibilit\'e ont \'et\'e modifi\'ees al\'eatoirement.
...
performance d'une solution est \'egalement mesur\'ee par sa flexibilit\'e c'est \`a
dire la facilit\'e avec laquelle on peut passer de cette solution \`a une
...
l'importance de chaque produit fini.       {\bf NDT}  A pr\'eciser ?
 ...
 l'affectation des operations aux machines.
...
\subsection{ Christian Artigues : gestion de projet ??}
...
\subsection{Ordonnancement cyclique}
...
L'\'etude de la robustesse
 est la recherche de garanties de performances sur
l'\ordo\ r\'ealis\'e.

{\bf ((} Nous proposons des outils
quantitatifs de mesure de la robustesse. Ces outils sont dtaills sur
\cite{versionweb}. Nous en donnons ici les descriptions informelles. Leur
utilisation effective dpend de la flexibilit accepte autant que de lint
rt de ces outils pour l'application considre. {\bf ))}


Nous notons ${\cal P}$ un probl\`eme statique, avec la description des
al\'eas potentiels (\'equivalent \`a un ensemble d'instances du probl\`eme
d\'eterministe).


Dans l'\'etude de la robustesse a priori, $\alpha$, $\delta$, ${\cal P}$
sont fix\'es. La mesure de robustesse du processus global
 calcule la distance entre l'\ordo\ ralis obtenu par le processus et
l'\ordo\ optimal, pour toutes les  instances de  ${\cal P}$. On considre
ensuite le maximum ou l'esprance de ces distances (la distribution
des diff\'erentes instances r\'ealis\'ees de ${\cal P}$ est alors suppose
connue).

La robustesse a posteriori d'un \ordo\ (au pire, en moyenne) mesure la robustesse
de l'\ordo\ fig\'e $\tilde
S$.{\bf ((}  Calculer cette robustesse revient  effectuer une tude de
 sensibilit sur $\tilde S$ ou l'algorithme qui le construit .{\bf ))}


On peut aussi limiter l'\'etude \`a la robustesse de l'algorithme statique
$\alpha$. Dans ce cas on recherche une garantie
	de performance v\'erifi\'ee pour tout algorithme dynamique (ou pour une
	famille raisonnable d'algorithmes dynamiques).

D'autres m\'etriques propos\'ees sont  :
\begin{itemize}
	\item la distance \`a une performance pr\'evue
(n\'egoci\'ee)  $\tilde z({\cal P})$.
    \item {\bf !} la mesure du niveau de service. Il s'agit de minimiser la
probabilit\'e que $z$ soit au dessus d'un certain seuil fix\'e $T({\cal
P})$.
\item  la mesure de la stabilit\'e d'un \ordo\ statique.
On cherche \`a minimiser l'\'ecart entre un \ordo\ privil\'egi\'e
calcul\'e en statique $\tilde S$,
 et celui r\'ealis\'e par le processus.

{\bf ((} On tend ici {\bf ))}  la notion de
stabilit\'e d'un \ordo\ issue de l'automatique, pour laquelle un \ordo\
reste stable s'il est inchang\'e en pr\'esence des al\'eas. Si l'\'ecart
calcul est un cart entre les performances des deux
\ordos, on retrouve la notion de stabilit\'e de l'algorithme {\bf ((}  statique
{\bf ))}  ou du
processus global (ref Sotskov).
\end{itemize}
