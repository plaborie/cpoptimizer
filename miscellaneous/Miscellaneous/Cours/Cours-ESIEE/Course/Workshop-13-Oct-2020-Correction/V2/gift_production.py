...
letterW   = [ [ [interval_var(optional=True,size=D[l]*W[k][1]) for l in o[0]] for o in O ] for k in range(m) ]
assembleW = [ [ interval_var(optional=True,size=A*len(o[0])*W[k][1]) for o in O] for k in range(m) ]
...
for k in range(m):
  model.add(no_overlap(assembleW[k] + [letterW[k][i][j] for i in range(n) for j in range(len(L[i]))]))

# Objective
tardiness = [ int_div(max(0,end_of(assemble[i])-int(0.5*mn(O[i][1]))), 24*60) for i in range(n)]
