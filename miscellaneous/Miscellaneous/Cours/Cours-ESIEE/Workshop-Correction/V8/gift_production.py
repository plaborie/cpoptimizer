...
# Maximal storage capacity for letters
StorageCapacity = 20 

# Maximal number of (same) letters that can be produced together
BatchCapacity = 4

# Setup-time (in mn) between two different letters
SetupTime = 24*60

# Weight for storage cost
Alpha = 0.1
...
M = [ [ SetupTime if j1!=j2 else 0  for j2 in range(len(D)) ] for j1 in range(len(D))]
...
stored    = [ [interval_var() for l in o[0]] for o in O ]

workerState  = [ state_function() for k in W]
machineState = [ state_function(M) for k in W]

storage  = sum(pulse(stored[i][j],1) for i in range(n) for j in range(len(L[i])))
capacity = [ sum(pulse(letterW[k][i][j],1) for i in range(n) for j in range(len(L[i]))) for k in range(m)]

# Constraints

model.add(storage <= StorageCapacity)

for i in range(n):
    for j in range(len(L[i])):
        #model.add(end_before_start(letter[i][j],assemble[i])) # This constraint is now redundant
        model.add(end_at_start(letter[i][j],stored[i][j]))
        model.add(end_at_start(stored[i][j],assemble[i]))
...
for k in range(m):
  ...
  for i in range(n):
      ...
      for j in range(len(L[i])):
          ...
          model.add(always_equal(machineState[k], letterW[k][i][j],I[L[i][j]],1,1))
...
earliness = [ int_div(max(0,mn(O[i][1])-end_of(assemble[i])), 24*60) for i in range(n)]
storage   = Alpha * int_div(sum(length_of(stored[i][j]) for i in range(n) for j in range(len(L[i]))), 24*60)
model.add(minimize(storage + sum(earliness[i] + square(tardiness[i]) for i in range(n))))
